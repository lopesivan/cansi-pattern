...
parser = optparse.OptionParser(
    usage="%prog -[ [s] --[save] ]", version="%prog 1.0")
...
def main():
    ...
    if os.path.isfile(path) and os.access(path, os.R_OK):
        ...
        data_model = yaml.safe_load(config_file)
        config_file.close()
        ...
        if len(data_model['state']['states']) > 0:
            for x in range(0, len(data_model['state']['states'])):
                t = models.TStateState_h(
                    data_model, 'cansi.state.StateState_h', x)
                if options.stdout == True:
                    t.put()
                if options.save == True:
                    t.save()

                t = models.TStateState_c(
                    data_model, 'cansi.state.StateState_c', x)
                if options.stdout == True:
                    t.put()
                if options.save == True:
                    t.save()
